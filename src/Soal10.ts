import promptSync from 'prompt-sync';  // Menggunakan import untuk prompt-sync
const prompt = promptSync();  // Membuat objek prompt untuk input dari pengguna

// Fungsi untuk menghitung indeks dua angka yang jumlahnya sama dengan target
function twoSum(nums: number[], target: number): number[] {
    const map = new Map<number, number>();  // Membuat hash map untuk menyimpan nilai dan indeksnya
